...
class PioneerNN(nn.Module):
    ...
    def load_weights_from_json(self, json_obj):
        ...
        if meta_input is not None and meta_input != self.input_size:
            raise ValueError(f"Le fichier a input_size={meta_input}, ce réseau a input_size={self.input_size}")
        if meta_hidden is not None and meta_hidden != self.hidden_size:
            raise ValueError(f"Le fichier a hidden_size={meta_hidden}, ce réseau a hidden_size={self.hidden_size}")
        if meta_output is not None and meta_output != self.output_size:
            raise ValueError(f"Le fichier a output_size={meta_output}, ce réseau a output_size={self.output_size}")
