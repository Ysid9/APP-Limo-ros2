...
# Entrées : err_x, err_y, cos(e_theta), sin(e_theta)
HL_size = 1000
input_size = 4
output_size = 2
...
if choice == 'y':
    try:
        with open('last_w_torch_4in.json') as fp:
            json_obj = json.load(fp)
        network.load_weights_from_json(json_obj)
        print("Weights loaded from last_w_torch_4in.json")
    except FileNotFoundError:
        print("No weight file found (last_w_torch_4in.json), starting with random weights.")
...
if save_choice.lower() == 'y':
    ...
    with open('last_w_torch_4in.json', 'w') as fp:
        json.dump(json_obj, fp)
    print("Weights saved to last_w_torch_4in.json")
else:
